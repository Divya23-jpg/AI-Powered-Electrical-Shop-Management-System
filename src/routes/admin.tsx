...
function AdminPage() {
  ...
  const refreshingSheet1 = useQuery({ ...sheet1Query, enabled: false }).isFetching;
        ...
        <div className="mt-6 rounded-xl border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border p-4">
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => qc.invalidateQueries({ queryKey: ["sheet1"] })}
              disabled={refreshingSheet1}
            >
              {refreshingSheet1 ? "Refreshing…" : "Refresh"}
            </Button>
          </div>
          {sheet1?.error && (
